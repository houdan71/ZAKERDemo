package com.example.dllo.zaker.playfun.location.city_glide;
...
public class MySlideView extends View {
    ...
    private void initView() {
        ...
        mBound = new Rect();
        //索引蓝背景颜色
        backgroundColor = getResources().getColor(R.color.back_gray_background);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < PlayFunLocationChooseActivity.letters.length; i++) {
            String textView = PlayFunLocationChooseActivity.letters[i];
            if (i == position - 1) {
                //d点击字母时候的按下颜色
                paint.setColor(getResources().getColor(R.color.ColorTitle));
                selectTxt = PlayFunLocationChooseActivity.letters[i];
                listener.showTextView(selectTxt, false);
            } else {
                //字母颜色
                paint.setColor(getResources().getColor(R.color.dark_gray_background));
            }
            //字体大小
            ...
            mTextHeight += mHeight / PlayFunLocationChooseActivity.letters.length;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                backgroundColor = getResources().getColor(R.color.back_gray_background);
                mTextHeight = mHeight / PlayFunLocationChooseActivity.letters.length;
                if (y / (mHeight / (PlayFunLocationChooseActivity.letters.length + 1)) < PlayFunLocationChooseActivity.letters.length - 5) {
                    position = y / (mHeight / (PlayFunLocationChooseActivity.letters.length + 1)) + 1;
                } else {
                    position = y / (mHeight / (PlayFunLocationChooseActivity.letters.length + 1));
                }
                invalidate();//重新绘制
                break;
            case MotionEvent.ACTION_MOVE:
